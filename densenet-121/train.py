    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if (not math.isnan(score) and score > best_score) or (
            ...
            torch.save(
                ...
                os.path.join(OUTPUT_DIR, "model_densenet121.pth"),
            )
            print("  --> Saved new best checkpoint")

        history.append({"epoch": epoch, "train_loss": train_loss, **metrics})

    pd.DataFrame(history).to_csv(os.path.join(OUTPUT_DIR, "model_densenet121_history.csv"), index=False)
    print("Done. Best checkpoint:", os.path.join(OUTPUT_DIR, "model_densenet121.pth"))
